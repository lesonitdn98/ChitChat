...
class UpdateViewModel(dataManager: DataManager, schedulerProvider: SchedulerProvider) :
        ...
        compositeDisposable.add(
            dataManager.getUser(dataManager.getAuthToken())
                ...
                .subscribe({
                    ...
                    navigator?.setState(it.user.email.email_state, it.user.dob.dob_state)
                    navigator?.setGender(it.user.gender)
                    dataManager.setCurrentUserAvatar(it.user.avatar)
                }) { throwable ->
                    setIsLoading(false)
                    navigator?.onMessage(throwable.toString())
                })
    }
    ...
    fun updateUserInfo(userUpdate: UserUpdate) {
        ...
        compositeDisposable.add(dataManager.update(dataManager.getAuthToken(), updateData)
            ...
            .observeOn(schedulerProvider.ui())
            .subscribe({
                navigator?.updateSuccess()
                getInfoUser()
                navigator?.onMessage(it.msg)
            }) { throwable ->
                setIsLoading(false)
                navigator?.onMessage(throwable.toString())
            })
