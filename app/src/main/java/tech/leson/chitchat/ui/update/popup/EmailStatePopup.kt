...
class EmailStatePopup @Inject constructor(private var activity: UpdateActivity) :
    ...
    override fun onClick(p0: View?) {
        when (p0?.id) {
            R.id.btnPublic -> activity.onEmailStateSelected(2)
            R.id.btnFriends -> activity.onEmailStateSelected(1)
            R.id.btnOnlyMe -> activity.onEmailStateSelected(0)
        }
    }
}